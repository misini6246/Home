...
class EloquentUserProvider implements UserProvider
{
    ...
    public function validateCredentials(UserContract $user, array $credentials)
    {
        ...
        if (!empty($authSalt)) {
            return $authPassword === md5(md5($plain) . $authSalt);
        } else {
...
trait AuthenticatesUsers
{
    ...
    public function postLogin(Request $request)
    {
        ...
        if ($request->ajax()) {
            return array('error' => 1, 'message' => $this->getFailedLoginMessage());
        }
    ...
    protected function handleUserWasAuthenticated(Request $request, $throttles)
    {
        ...
        if ($request->ajax()) {
            return array('back' => $back, 'error' => 0);
        }
    ...
    public function getLogout()
    {
        if (auth()->check()) {
            ...
            Cache::tags(array('miaosha', '20160824', 'users'))->forget($user->user_id);
            Cache::tags(array('miaosha', '20160824', 'ywy_zq'))->forget($user->user_id);
        }
        Auth::logout();
...
trait ResetsPasswords
{
    ...
    {
        return view('auth.password')->withTitle('密码找回');
    }
    public function postEmail(Request $request)
    {
        $this->validate($request, array('email' => 'required|email', 'user_name' => 'required'));
        $response = Password::sendResetLink($request->only('email', 'user_name'), function (Message $message) {
    ...
    {
        return property_exists($this, 'subject') ? $this->subject : '密码重置链接';
    }
    public function getReset($token = null)
    {
        ...
        return view('auth.reset')->with(array('token' => $token, 'title' => '重置密码'));
    }
    ...
    protected function resetPassword($user, $password)
    {
        if (!empty($user->ec_salt)) {
            $user->password = md5(md5($password) . $user->ec_salt);
        } else {
...
{
    protected $providers = array(FormRequestServiceProvider::class);
}
}
...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    {
        return $this->hasCast($key) && in_array($this->getCastType($key), array('date', 'datetime'), true);
    }
    protected function isJsonCastable($key)
    {
        return $this->hasCast($key) && in_array($this->getCastType($key), array('array', 'json', 'object', 'collection'), true);
    }
