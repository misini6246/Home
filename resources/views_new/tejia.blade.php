...
@section('content')
    ...
    @include('layouts.nav')
    @include('tejia.gg1',['page'=>$pages->currentPage()])
    @if(isset($ad160))
                                ...
                                @foreach($pages as $k=>$v)
                                    <li class="child" style="height: 391px;">
                                        ...
                                        <a href="{{$v->goods_url}}" target="_blank" class="child_a">
                                            <img class="fly_img{{$v->goods_id}} lazy"
                                                 data-original="{{empty($v->goods_thumb)? 'no_picture.gif':$v->goods_thumb}}"
                                                 alt=""/>
                                   ...
                                   @if($v->is_can_see==0) 会员可见 @else
                                                    ...
                                                    <li><span class="title"
                                                              style="color: red">活动时间：{{$ad160->ad_name or '07.26'}}</span>@if(($v->xg_type>0&&$v->xg_end_date>$v->promote_start_date&&$v->xg_start_date<=$v->promote_start_date)||$v->xg_type==1)
                                                            <span
                                                                  class="title">限量：{{$v->ls_ggg}}</span>@endif</li>
                                                @else
                                                    <li><span class="title">价格：</span><span
                                                                style="width: 69px;display: inline-block" class="price">
                                   @if($v->is_can_see==0) 会员可见 @else
                                                                {{formated_price($v->real_price)}}
                                                            @endif
                                </span>@if(($v->xg_type>0&&$v->xg_end_date>time()&&$v->xg_start_date<time())||$v->xg_type==1)
                                                            <span class="title">限&nbsp;&nbsp;&nbsp;量：{{$v->ls_ggg}}</span>@endif
                                                    </li>
                                                    <li><span style="width: 100px;display: inline-block" class="title">库存：@if($v->goods_number>800)
                                                                充裕@elseif($v->goods_number==0)
                                                                缺货@else{{$v->goods_number}}@endif</span><span
                                                                class="title">@if($v->zbz)
