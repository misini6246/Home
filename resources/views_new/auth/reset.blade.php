...
<div class="main_2">
    ...
    <form action="{{route('pwd_reset')}}" method="post" name="getPassword2" class="refer" onsubmit="return yzmm()">
        {!! csrf_field() !!}
        ...
        {{--<p>--}}
        {{--<input class="psw2" type="text" name="email" value="{{$user_name or old('email')}}"/>--}}
    ...
    width: 110px;
    height: 40px;background-image: none;line-height: 40px;" type="submit" class="psw_btn" value="确认密码"/>
        </div>
...
<script>
    function yzmm() {
        ...
        var is_mm = /^[a-zA-Z\d_]{6,24}$/;
        if (!is_mm.test(mm)) {
            layer.msg('密码只能包含大小写数字及下划线,最短六位', {icon: 2});
            return false;
        }
        var c_mm = $('input[name=password_confirmation]').val();
        if (c_mm != mm) {
