        ...
        <div class="zhongyyp-box fn_clear">
                                ...
                                @foreach(cate_tree(445) as $k=>$v)
                                    @if($k<7)
                                        <li @if($phaid==$v->cat_id)class="on_click"@endif ><a
                                                    href="{{build_url('category.zyyp',['pid'=>$v->cat_id,'product_name'=>$product_name_here,'zm'=>$zmhere])}}">{{$v->cat_name}}</a>
                                        </li>@endif
                                @endforeach
                            </ul>
                                ...
                                @foreach($sccj as $k=>$v)
                                    @if($k<4)
                                        <li @if($product_name_here==$v)class="on_click"@endif ><a
                                                    href="{{build_url('category.zyyp',['pid'=>$phaid,'product_name'=>$v,'zm'=>$zmhere])}}">{{$v}}</a>
                                        </li>@endif
                                @endforeach
                        ...
                        <div class="initial-a">
                            ...
                            @foreach($zm as $v)
                                <a @if($zmhere==$v)class="checked_a"
                                   @endif href="{{build_url('category.zyyp',['pid'=>$phaid,'product_name'=>$product_name_here,'zm'=>$v])}}">{{$v}}</a>
                            @endforeach
                        </div>
                                ...
                                <li>
                                    <div class="shoucang">
                                        <a href="javascript:collect({{$v->goods_id}})"><img
                                                    src="./images/zyyp/zhyp063.png" alt=""/></a>
                                    </div>
                                    @if($v->is_hot==1)
                                        <div class="hot">
            ...
            @else
                <div class="g_right_bottom">
                    <p>抱歉, 没有找到相关的药品,</p>
                    <p><a href="requirement.php" target="_blank">点击这里提交求购意向，{{config('services.web.name')}}网会尽快补货！</a></p>
                </div>
                <div class="g_right_bottom_bottom">
                    <p>没有找到你想要的药品？ <a href="requirement.php" target="_blank">点击这里提交求购意向，{{config('services.web.name')}}网会尽快补货！</a></p>
                </div>
            @endif
        </div>
