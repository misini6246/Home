    ...
    <div class="content_2" style="position: relative;">
        ...
        <div class="info-box">
            ...
            <div class="info">
                ...
                <a href="{{route('user.orderInfo',['id'=>$order['order_id']])}}" target="_blank"
                   class="important-info">{{$order['order_sn']}}</a>&nbsp; &nbsp; &nbsp;
                        @if(isset($order['order_sn_sy']))
                            <a href="{{route('user.orderInfo',['id'=>$order['order_id_sy']])}}" target='_blank'
                               class="important-info">{{$order['order_sn_sy']}}(血液制品订单)</a>&nbsp; &nbsp; &nbsp;
                        @endif
                        @if(isset($order['order_sn_mhj']))
                            <a href="{{route('user.orderInfo',['id'=>$order['order_id_mhj']])}}" target='_blank'
                               class="important-info">{{$order['order_sn_mhj']}}(麻黄碱订单)</a>
                        @endif
            </span></p>
                <p><span class="box"><span class="title">商品总金额：</span><span
                                class="important-info price_txt">{{formated_price($order['goods_amount'])}}</span></span>
                </p>
                <p @if($order['shipping_fee']<=0) style="display: none;" @endif id='shipping_fee'><span
                            class="box"><span class="title">物流费用：</span><span class="important-info price_txt">{{formated_price($order['shipping_fee'])}}
                            (四川省内单张订单满800包邮)</span></span></p>
                <p><span class="box"><span class="title">折扣金额：</span><span
                                class="important-info price_txt">{{formated_price($order['discount'])}}</span></span>
                </p>
                <p><span class="box"><span class="title">优惠金额：</span><span
                                class="important-info price_txt">{{formated_price($order['zyzk'])}}</span></span></p>
                @if($order['surplus']>0)
                    <p><span class="box"><span class="title">使用余额：</span><span
                                    class="important-info price_txt">{{formated_price($order['surplus'])}}</span></span>
                    </p>
                @endif
                @if($order['pack_fee']>0)
                    <p><span class="box"><span class="title">使用优惠券：</span><span
                                    class="important-info price_txt">{{formated_price($order['pack_fee'])}}</span></span>
                    </p>
                @endif
                @if(isset($order['jf_money'])&&$order['jf_money']>0)
                    <p><span class="box"><span class="title">使用积分金币：</span><span
                                    class="important-info price_txt">{{formated_price($order['jf_money'])}}</span></span>
                    </p>
                @endif
                @if(isset($order['cz_money'])&&$order['cz_money']>0)
                    <p><span class="box"><span class="title">使用{{trans('common.cz_money')}}：</span><span
                                    class="important-info price_txt">{{formated_price($order['cz_money'])}}</span></span>
                    </p>
                @endif
                @if($order['jnmj']>0)
                    <p><span class="box"><span class="title">使用{{trans('common.jnmj')}}：</span><span
                                    class="important-info price_txt">{{formated_price($order['jnmj'])}}</span></span>
                    </p>
                @endif
                <p><span class="box"><span class="title">应付总金额：</span><span
                                class="important-info price_txt">{{formated_price($order['order_amount'])}}</span></span>
                </p>
...
line-height: 28px;">温馨提示：</span>
                    <p style="line-height: 28px;">为保证你所选的商品库存，请尽快付款，未付款订单系统会在 <span
                                class="important-info">48小时以后 </span>自动取消。</p>
                    <p style="line-height: 28px;color: red;margin-left: 83px;">为了您的货款安全，请不要将货款转到公司指定以外的账户！</p>
                    {{--<p style="float: left;line-height: 28px;color: red;margin-left: 83px;">{{trans('common.jnmj')}}低于200将自动转入账户余额，详情可咨询客服或前往余额管理查看账户明细！</p>--}}
                    @if(isset($order['mhj_tip'])&&!empty($order['mhj_tip']))
                        <p style="line-height: 28px;color: red;margin-left: 83px;">
                            {{$order['mhj_tip']}}</p>
                    @endif
                </div>
                {{--@if(isset($order['order_sn_mhj']))--}}
        ...
        @if($order['pay_name']=='银行汇款/转帐')
            <div class="bank-list">
                ...
                <img style="margin-left: -60px;" src="{{get_img_path('images/yinhangxinxi.png')}}2">
            </div>
        @endif
    </div>
            ...
            <div class="mark_box">
                ...
                <div class="info_m">
                    ...
                    <p class="text"><span class="suc_ico ico"></span> 支付成功： <a
                                href="{{route('user.orderList')}}">查看订单</a> <a href="{{route('index')}}">继续购物</a></p>
                    <p class="text"><span class="fail_ico ico"></span> 支付失败： <a
                                href="{{route('articleInfo',['id'=>49])}}">查看支付帮助</a></p>
                </div>
