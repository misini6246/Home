    ...
    <div class="container" id="user_center">
        <div class="container_box">
            ...
            <div class="right">
                ...
                @if(count($result)>0)
                    <form id="search_form" name="search_form" action="{{route('member.zq_order.index')}}">
                    </form>
                    <table>
                        <tr>
                            <th class="ddh">订单号</th>
                        ...
                        @foreach($result as $v)
                            <tr>
                                <td class="ddh"><a style="display: block;color: #e70000"
                                                   href="{{route('member.zq_order.show',['id'=>$v->zq_id,'type'=>$v->zq_type])}}">{{$v->order_sn}}</a>
                                </td>
                                ...
                                <td>
                                    @if($v->pay_status==0&&$v->order_status=1)
                                        <a href="{{route('member.zq_order.show',['id'=>$v->zq_id,'type'=>$v->zq_type])}}"
                                           class="cz">去付款</a>
                                    @endif
                                    <a href="{{route('member.zq_order.show',['id'=>$v->zq_id,'type'=>$v->zq_type])}}"
                                       class="chakan">查看详情</a>
