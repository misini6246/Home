    ...
    <div class="main fn_clear">
                    ...
                    @foreach($weekSales as $k=>$v)
                        <li @if($k==0)class="first"@endif>
                            <a href="{{$v->goods_url}}" class="list">
                                ...
                                <span class="norms" alt="" title="{{$v->spgg or ''}}">{{str_limit($v->spgg,8)}}</span>

                            </a>
                            <span class="span_hide">
						<span class="com_num red">{{$k+1}}</span>
						<a href="{{$v->goods_url}}"><img
                                    src="{{empty($v->goods_thumb)? 'no_picture.gif':$v->goods_thumb}}"
                                    alt="{{$v->goods_name}}" title="{{$v->goods_name}}"/></a>
        ...
        <div class="right_list">
            @if($step!='nextpro'&&$step!='promotion'&&$step!='gzbl_promotion'&&$step!='gzbl_nextpro')
                @if($dis==2&&$ad163)
                    <a target="_blank" href="{{$ad163->ad_link}}"><img style="width: 100%;"
                                                                       src="{{$ad163->ad_code}}"></a>
                @endif
                <div class="r_top">
                    ...
                    <div class="g_csize fn_clear">
                        ...
                        <div class="g_listdiv g_fsdiv">
                            @foreach($select_arr as $v)
                                <a style="*margin-top:0;" class="g_select_cate" href="{{$v['url']}}"><h5>{{$v['name']}}
                ...
                <div class="list_title fn_clear">
                    <div class="ul_list1" style="display: block;">
                        ...
                        <ul>
                            @foreach($jx as $k=>$v)
                                @if($k<11)
                                    <li @if($jxhere==$v->jx) class="on_click" @endif ><a
                                                href="{{$url}}&jx={{$v->jx}}&zm={{$zmhere}}&sort={{$sorthere}}&order={{$order}}">{{$v->jx}}</a>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
                    ...
                    <div class="initial">
                        ...
                        @foreach($zm as $k=>$v)
                            <a @if($zmhere==$v) class="checked_a"
                               @endif href="{{$url}}&jx={{$jxhere}}&zm={{$v}}&sort={{$sorthere}}&order={{$order}}">{{$v}}</a>
                        @endforeach
                    </div>

                    <div class="search">
                        <form action="{{url('/category')}}" method="get">
                            <p><span class="title">药品名称：</span> <input
                                        style=" color: black;text-align: left;padding-top: 0px;" type="text"
                                        name="goods_name"
                                        value="{{$goods_name_here}}" class="text"/></p>
                            <p><span class="title">生产厂家：</span> <input
                                        style=" color: black;text-align: left;padding-top: 0px;" type="text"
                                        name="product_name"
                                        value="{{$product_name_here}}" class="text"/></p>
                    ...
                    <table class="table1">
                        <tr class="first">
                            <td class="td com @if($sorthere=='goods_id') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=goods_id&order={{$goods_id['next_order'] or 'asc'}}&showi={{$showi}}">综合
                                    <span class="{{$goods_id['order_class'] or 'arrow'}}"></span></a></td>
                            <td class="td com @if($sorthere=='click_count') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=click_count&order={{$click_count['next_order'] or 'asc'}}&showi={{$showi}}">人气
                                    <span class="{{$click_count['order_class'] or 'arrow'}}"></span></a></td>

                            <td class="td com @if($sorthere=='sales_volume') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=sales_volume&order={{$sales_volume['next_order'] or 'asc'}}&showi={{$showi}}">销量
                                    <span class="{{$sales_volume['order_class'] or 'arrow'}}"></span></a></td>

                            <td class="td com @if($sorthere=='shop_price') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=shop_price&order={{$shop_price['next_order'] or 'asc'}}&showi={{$showi}}">价格
                                    <span class="{{$shop_price['order_class'] or 'arrow'}}"></span></a></td>
                            <td class="td com @if($sorthere=='goods_name') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=goods_name&order={{$goods_name['next_order'] or 'asc'}}&showi={{$showi}}">品名
                                    <span class="{{$goods_name['order_class'] or 'arrow'}}"></span></a></td>
                            <td class="td com @if($sorthere=='product_name') sort @endif"><a
                                        href="{{$url}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort=product_name&order={{$product_name['next_order'] or 'asc'}}&showi={{$showi}}">厂家
                                    <span class="{{$product_name['order_class'] or 'arrow'}}"></span></a></td>
                            <td class="td com @if($step=='zyzk') sort @endif"><a
                                        href="{{route('category.index',['step'=>'zyzk'])}}">促销产品</a></td>
                            <td class="td7">
                                @if($dis!=7)
                                    <a href="{{$url}}&page={{$pages->currentPage()}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort={{$sorthere}}&order={{$order}}&step={{$step}}&showi=0"
                                       class="big_pic @if($showi!=0) checked @endif"></a>
                                    <a href="{{$url}}&page={{$pages->currentPage()}}&jx={{$jxhere}}&zm={{$zmhere}}&goods_name={{$goods_name_here}}&product_name={{$product_name_here}}&sort={{$sorthere}}&order={{$order}}&step={{$step}}&showi=1"
                                       class="list @if($showi==1) checked @endif"></a>
                                @endif
                            </td>
                                            ...
                                            <div style="position:relative;" class="t-tip">
                                                ...
                                                <span class="tip_span"
                                                      style="width:144px;height:46px;position:absolute;left:0px;top:21px;display:none;z-index:999;"><img
                                                            src="{{get_img_path('images/tj_tip.png')}}" alt=""/></span>
                            ...
                            <a href="{{$v->goods_url}}" class="a_img">
                                ...
                                <span class="info_txt">总销量: <span class="num">{{$v->sales_volume or 0}}</span></span>
                                    ...
                                    <td class="table2_td11"><a
                                                ...
                                                @endif  id="sssss_{{$v->goods_id}}"
                                                class="add_cart fly_to_cart{{$v->goods_id}}">加入购物车</a> <a
                                                @if($v->is_can_see==1)href="javascript:collect({{$v->goods_id}})"
                                                @else href="javascript:addToCart2({{$v->goods_id}})"
                                                @endif class="col">收藏</a></td>
                            ...
                            @foreach($pages as $k=>$v)
                                <li class="child">
                                    ...
                                    <a href="{{$v->goods_url}}" target="_blank" class="child_a">
                                        <img class="fly_img{{$v->goods_id}}"
                                             src="{{empty($v->goods_thumb)? 'no_picture.gif':$v->goods_thumb}}" alt=""/>
                                        <a href="javascript:collect({{$v->goods_id}})" class="collect"><span
                                    ...
                                    <div class="box_goods">
                                        <ul>
                                            <li><a href="{{$v->goods_url}}" target="_blank" alt="{{$v->goods_name}}"
                                                   title="{{$v->goods_name}}">{{str_limit($v->goods_name,26)}}</a></li>
                                   ...
                                   @if($v->is_can_see==0) 会员可见 @else
                                                ...
                                                <li><span class="title">库存：@if($v->goods_number>800)
                                                            充裕@elseif($v->goods_number==0)
                                                            缺货@else{{$v->goods_number}}@endif &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;@if($v->zbz)
            ...
            @else
                <div class="g_right_bottom">
                    <p>抱歉, 没有找到
                        ...
                        @endif相关的药品,</p>
                    <p><a href="{{url('requirement')}}" target="_blank">点击这里提交求购意向，{{config('services.web.name')}}网会尽快补货！</a></p>
                </div>
                <div class="g_right_bottom_bottom">
                    <p>没有找到你想要的药品？ <a href="{{url('requirement')}}" target="_blank">点击这里提交求购意向，{{config('services.web.name')}}网会尽快补货！</a></p>
                </div>
            @endif
