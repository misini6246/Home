...
@section('content')
	...
	<div id="xiangqing" class="container">
		<div class="container_box">
			<div class="xiangqing_title">
				...
				<a href="{{route('index')}}">首页</a><i class="myicon xq_right"></i>{{$info->goods_name}}
			</div>
			<div class="xq_main">
				<div class="xq_main_top">
					...
					<div class="xq_datu">
							<span class="jqzoom" data-src="{{$info->goods_img}}">
								<img src="{{$info->goods_thumb}}"/>
							</span>
					</div>
					<div class="mid" style="position: relative;">
								<span
									style="position: absolute;height: 50px;bottom: -10px;font-size: 14px;font-weight: bold;line-height: 25px;left: -451px;width: 398px;text-align: center;background: #fff;">
						...
						@endif
						<div class="money">
							@if(strstr($info->show_area,'2')&&$info->market_price>0)
							...
							@if($info->is_cx==1)
								<p class="cxj">
									...
									@if($info->zyzk>0)
										<span class="bs" style="background: #418ed2">满减</span>
									@endif
									@if($info->is_zx==1)
							...
							@elseif($info->product_id==1)
								<p class="cxj">
									促销价：<span>{{$info->real_price_format}}
										...
										@endif</span><span class="bs" style="padding: 0 5px;width: auto;">此品种为特卖品种</span>
									@if($info->is_xqpz==1)
										<span class="bs" style="padding: 0 5px;width: auto;">此品种为效期品种</span>
									@endif
								</p>
							{{-- 如果商品属于精品专区且建议零售价大于0，则显示建议零售价 --}}
							@else
								<p class="cxj">
							...
							@endif
							@if($user&&$user->ls_review==1&&$info->zyzk>0&&time()>$info->preferential_start_date&&time()<$info->preferential_end_date)
								<p style=" color:#ff2828;margin-top:8px;font-size:16px">每满￥{{$info->shop_price}}减￥{{$info->zyzk}}</p>
							@endif
								@if($info->is_yhq_status == 2)
									<span class="jiesuan-span" style=" color:#ff2828;display: inline-block;height: 30px;line-height: 30px;overflow: hidden">此商品不参与优惠券活动及返利活动</span>
								@endif
								@if($info->is_yhq_status == 1)
									@if($info->is_promote == 1 && time() >= $info->promote_start_date && time() < $info->promote_end_date)
										<span class="jiesuan-span" style=" color:#ff2828;display: inline-block;height: 30px;line-height: 30px;overflow: hidden">此商品不参与优惠券活动及返利活动</span>
									@endif

									@if(time() >= $info->preferential_start_date && time() < $info->preferential_end_date && $info->zyzk > 0.01)
										<span class="jiesuan-span" style=" color:#ff2828;display: inline-block;height: 30px;line-height: 30px;overflow: hidden">此商品不参与优惠券活动及返利活动</span>
									@endif
						...
						<div class="text">
							...
							<p><span class="justy">件装量</span>：{{$info->jzl}}</p>
						...
						<div class="jiajian">
							...
							<span class="jian">-</span><input @if($info->product_id==0)
									...
									data-jzl="{{$info->jzl}}"
									data-xl="{{$info->xg_num}}"
									data-isxl="{{$info->is_xg}}"/><span class="jia">+</span>库存：@if($info->goods_number>800)
			...
			<div class="xiangqing_bottom">
				...
				<div>
					<p>
						<strong>
							以下商品说明书由今瑜e药网手工录入，可能会与实际有所误差，仅供参考，具体请以实际商品为准
						</strong>
					</p>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		// 剩余时间
		var leftTime={{$info->promote_end_date-time()}};
		if(leftTime>0){
			var showLeft=setInterval(function(){
				var leftH=parseInt(leftTime/60/60%24)
				var leftM=parseInt(leftTime/60%60)
				var leftS=parseInt(leftTime%60)
				if(leftH<10){
					leftH='0'+''+leftH
				}
				if(leftM<10){
