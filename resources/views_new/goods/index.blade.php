...
@section('content')
	...
	<div class="goods-box" @if(isset($keywords)&&!empty($keywords)&&count($result)>0) style="min-height: 1302px" @endif>
		...
		<div class="goods-box-right">
			{{--@if(isset($dis)&&$dis==2)--}}
			{{--<a target="_blank" href="/tg/jpzq1807">--}}
			{{--<img style="margin-top: 10px;" src="{{get_img_path('adimages1/201808/jpzqlj.jpg')}}">--}}
			{{--</a>--}}
			{{--@endif--}}
			{{-- 判断是否精品专区，如果是则显示精品专区banner --}}
			@if(isset($dis)&&$dis==2)
				<a href="">
					<img style="margin-top: 10px;transform:none;-webkit-transform:none;width:100%" src="{{get_img_path('images/jpzq/banner.png')}}">
				</a>
			@endif
											...
											<td class="cxbs">
												...
												@if($v->zyzk>0)
													...
													<span class="yh_bs layer_tips" id="lb_yh{{$v->goods_id}}"
														  data-msg="￥{{$v->zyzk}}">优惠<i class="jiantou xia_i"></i></span>
												@endif
												@if($v->is_zx==1)
