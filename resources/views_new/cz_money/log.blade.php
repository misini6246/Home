...
@section('content')
    ...
    <div class="main fn_clear">
        <div class="top"><span class="title">我的太星医药网</span> <a>>　<span>我的账户</span> </a> <a
                    href="{{route('user.account')}}" class="end">>　<span>{{trans('common.cz_money')}}</span></a></div>
        @include('layout.user_menu')
        <div class="main_right1">
            <div class="top_title">
                <h3>{{trans('common.cz_money')}}</h3>
                <span class="ico"></span>
            ...
            <table class="table2">
                ...
                @foreach($result as $v)
                    <tr>
                        ...
                        <td class="tb2_td2">@if($v->money>0)增加@else减少@endif</td>
                        <td class="tb2_td3">{{formated_price(abs($v->money))}}</td>
                        <td class="tb2_td4" title="{$item.change_desc}">{!! $v->change_desc !!}</td>
                    </tr>
                @endforeach
                <tr>
                    <td colspan="4" class="al_right"><p class="balance" style="font-size: 16px;">
                            您当前的可用{{trans('common.cz_money')}}
                            为:{{formated_price(collect($cz_money)->get('money',0))}}</p></td>
                </tr>
            </table>
            @if($result->lastPage()>0)
                {!! pagesView($result->currentPage(),$result->lastPage(),3,3,['url'=>'cz_money.log']) !!}
            @endif
