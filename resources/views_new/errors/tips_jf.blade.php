...
@section('links')
    <meta http-equiv="refresh" content="3;URL={{$link or route('jifen.index')}}"/>
    <link href="{{path('css/jifen/common.css')}}" rel="stylesheet" type="text/css"/>
    <link href="{{path('css/jifen/index.css')}}" rel="stylesheet" type="text/css"/>
            ...
            <div class="box_1">
                ...
                <div class="boxCenterList RelaArticle" align="center">
                    <div style="margin:20px auto;">
                        <p style="font-size: 14px; font-weight:bold; color: red;">{{$exception->getMessage()}}</p>
                        @if(count($exception->getLinks())==0)
                            <p><a href="{{route('jifen.index')}}">返回积分商城</a></p>
                        @else
