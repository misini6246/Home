...
@section('content')
    <div class="sure_order_real" style="padding: 0;border: 0;">
        <div class="fill_msg" style="display: block;">
            <form action="{{route('jifen.address.update',['id'=>$info->id])}}" method="post" id="form">
                {!! method_field('put') !!}
                {!! csrf_field() !!}
                <label class="clear_float fill_name"><span class="ca_tip">收货人姓名：</span>
                    <input class="people" type="text" name="consignee" value="{{$info->true_name}}"/>
                ...
                <div class="choose_address clear_float">
                    ...
                    <div class="select choose_select province">
                        <div class="select_choose">
                            <span data-id="{{$info->province}}">{{collect($info->regionProvince)->get('region_name')}}</span><i></i>
                        </div>
                    ...
                    <div class="select choose_select city">
                        <div class="select_choose">
                            <span data-id="{{$info->city}}">{{collect($info->regionCity)->get('region_name')}}</span><i></i>
                        </div>
                        <ul class="select_options">
                            <li data-id="0" style="border: 0">请选择...</li>
                            @foreach($city as $v)
                    ...
                    <div class="select choose_select district">
                        <div class="select_choose">
                            <span data-id="{{$info->area}}">{{collect($info->regionDistrict)->get('region_name')}}</span><i></i>
                        </div>
                        <ul class="select_options">
                            <li data-id="0" style="border: 0">请选择...</li>
                            @foreach($area as $v)
                ...
                <label class="clear_float">
                    ...
                    <textarea class="detail_address" cols="80" rows="10" name="address">{{$info->address}}</textarea>
                    <span class="alert">*</span>
                </label>
                <label class="clear_float">
                    <span class="ca_tip">邮政编码：</span>
                    <input class="postcode" type="text" name="zipcode" value="{{$info->zip_code}}"/>
                    <span class="alert">*</span>
                </label>
                <label class="clear_float cellphone">
                    <span class="ca_tip">电话：</span>
                    <input class="cellphone_num" type="text" name="tel" value="{{$info->mob_phone}}"/>
                    <span class="alert">*</span>
                </label>
                <div class="submit">
                    <input type="hidden" name="province" id="province" value="{{$info->province}}"/>
                    <input type="hidden" name="city" id="city" value="{{$info->city}}"/>
                    <input type="hidden" name="district" id="district" value="{{$info->area}}"/>
                    <button class="reset_btn save">确认提交</button>
