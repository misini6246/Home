...
<div class="tanchuc" @if($show==0)style="display:none;"@endif id="shopping_box">
    <div class="content_buy" ><a href="javascript:;" @if($error==0)class="success_img"@else class="error_img" @endif></a>
        <h4 class="tip_text">{{$text or '错误'}}</h4>
        <p class="tip_txt" alt="" title="">{{$content or ''}}</p>
        @if($show1==0)
            <p class="login_p tab_p1" >
                <a href="{{$show1_url1 or 'javascript:;'}}" class="login_a again" >{{$show1_btn1 or '继续购物'}}</a> <a href="{{$show1_url2 or route('cart.index')}}">{{$show1_btn2 or '去结算'}}</a>
            </p>
        @else
            <p class="login_p tab_p4" >
                <a href="{{$show1_url3 or 'javascript:;'}}" class="login_a confirm again">{{$show1_text3 or '确认'}}</a>
            </p>
        @endif
