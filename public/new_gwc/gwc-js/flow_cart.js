...
function add_num(rec_id){
    ...
    var num = parseFloat(_th.parents("tr").find('#goods_num_show_'+rec_id).val()) ;
...
function changePrice_ls(rec_id){
	...
	var _th=$(".gwc_tb2 input[name=newslist]") ;
    var num = parseFloat(_th.parents("tr").find('#goods_num_show_'+rec_id).val()) ;
    var zbz = parseFloat(_th.parents("tr").find('#goods_num_show_'+rec_id).attr('data-zbz'));
...
function updateCartResponse(result){
    if (result.error == 0){
		...
		$('#zp_goods'+result.rec_ids).replaceWith(result.child);
		// 2015-7-9 换购
