...
$(function () {
    ...
    function testUsername($this) {//验证用户名
        ...
        if (testNull($this.val())) {
            $this.next(".prompt").html("·请输入用户名");
            $this.next(".prompt").css("color", "red");
            return false;
        }
//        else if (!(/^[A-Za-z0-9_]{0,24}$/.test($(this).val()))) {
//            $(this).next(".prompt").html("·用户名含有非法字符！");
//            $(this).next(".prompt").css("color", "red");
//        }
        else if (!(/^[\d\D]{2,30}$/.test($this.val()))) {
            $this.next(".prompt").html("·用户名只能是2到30个字符！");
            $this.next(".prompt").css("color", "red");
            return false;
        } else {
            //查询用户名是否被注册
            ...
            $.ajax({
                ...
                success: function(m){
                    ...
                    if(mm.c){
                        ...
                        $this.next(".prompt").css("color", "green");
                        is=true;
                    }else{
                        $this.next(".prompt").html("该用户名已被注册！");
                        $this.next(".prompt").css("color", "red");
                    }
                }
            });
    ...
    function testPassword($this) {//验证密码
        if (testNull($this)) {
            $this.next(".prompt").html("·请输入密码");
            $this.next(".prompt").css("color", "red");
            return false;
        } else if (/^[A-Za-z0-9_]{0,5}$/.test($this.val())) {
            $this.next(".prompt").html("·密码不能少于6位");
            $this.next(".prompt").css("color", "red");
            return false;

        } else if (/^[A-Za-z0-9_]{25,}$/.test($this.val())) {
            $this.next(".prompt").html("·密码不能超过24位");
            $this.next(".prompt").css("color", "red");
            return false;
        } else if (!(/^[A-Za-z0-9_]{6,24}$/.test($this.val()))) {
            $this.next(".prompt").html("·请输入6-24位英文、数字、\"_\"的组合字符");
            $this.next(".prompt").css("color", "red");
            return false;

        } else if ($this.val() == sure_pass.val() && sure_pass.val() != "") {
            $this.next(".prompt").html("√");
            sure_pass.next(".prompt").html("√");
            $this.next(".prompt").css("color", "green");
            sure_pass.next(".prompt").css("color", "green");
            return true;
        } else {
            $this.next(".prompt").html("√");
            $this.next(".prompt").css("color", "green");
        ...
        if($this.val() != sure_pass.val() && sure_pass.val() != ""){
            sure_pass.next(".prompt").html("·确认密码与密码不一致！");
            sure_pass.next(".prompt").css("color", "red");
            return false;
        }
    }
    ...
    function testEmail($this) {//验证电子邮箱
        if (testNull($this)) {
            $this.next(".prompt").html("·请输入电子邮箱");
            $this.next(".prompt").css("color", "red");
            return false;

        } else if (!(/^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/.test($this.val()))) {
            $this.next(".prompt").html("·电子邮箱格式不正确！");
            $this.next(".prompt").css("color", "red");
            return false;
    ...
    function testQQ($this) {//验证qq
        if ($this.val() != "") {
            if (!(/^[1-9][0-9]{4,11}$/.test($this.val()))) {
                $this.next(".prompt").html("·请输入正确的QQ");
                $this.next(".prompt").css("color", "red");
                return false;
            } else {
				//查询QQ是否已存在
    ...
    function testPhone($this) {//验证电话
		if (testNull($this)) {
            ...
            if (!(isPhone.test($this.val()) || isMob.test($this.val()))) {
                $this.next(".prompt").html("·请输入正确的电话号码！");
                $this.next(".prompt").css("color", "red");
                return false;
            } else {
                //查询电话是否已存在
    ...
    psw2.focusin(function(){
        if($(this).val()!=""){
            testPassword($(this));
        }else{
            $(this).next(".prompt").html("·请输入6-24位英文、数字、\"_\"组成的密码");
            $(this).next(".prompt").css("color", "#666");
        }
    });
...
//        验证所在地区下拉框
       ...
       //end
        if(select1&&select2&&select3){
			...
			$(".choose_address .prompt").css("color", "red");
            return true
        }else{
			$(".choose_address .prompt").css("color", "red");
	...
	//地址在这里第一个参数改
    $(".province .select_options li").live("mouseup",function(){select_area("user.php",$(this),".city");return false});
    $(".city .select_options li").live("mouseup",function(){select_area("user.php",$(this),".district");return false});
    /**
     *
     * @param url   请求地址
     * @param e     当前操作的节点
    ...
    function select_area(url,e,where){
        ...
        if(id){
            $.ajax({type:"get",url:url,data:"act=get_regions&id="+id,success:function(msg){
    ...
    username.focusin(function(){
        if($(this).val()!=""){
            ...
            $(this).next(".prompt").html("·请输入2-10个字符组成的用户名");
            $(this).next(".prompt").css("color", "#666");
        }
    ...
    email.focusin(function(){
        if($(this).val()!=""){
            ...
            $(this).next(".prompt").html("·请输入有效的电子邮箱");
            $(this).next(".prompt").css("color", "#666");
        }
    });
    enterprise.focusin(function(){
        if($(this).val()!=""){
            testEnterprise($(this));
        }else{
            $(this).next(".prompt").html("·请填写与营业执照上名称一致，以便尽快通过审核");
            $(this).next(".prompt").css("color", "#666");
        }
    });
    /*邮箱提示*/
    ...
    $("#submit").click(function (e) {//点击注册
        if ($(".agree input").attr("checked")) {
			...
			var sure_pass=$("input[name='confirm_password']");
			var email=$("input[name='email']");
			var enterprise=$("input[name='extend_field1']");
			var qq=$("input[name='extend_field2']");
			var phone=$("input[name='extend_field5']");
			...
			//alert(testUsername(username));
            if (mark) {
				var ls_lx = $("input[type=radio]:checked").attr("value");  //2015-6-10
				var ls_name=$("input[name='ls_name']").val();
				var username=username.val();
				var password=password.val();
				var email=email.val();
				var extend_field1=enterprise.val();
				var extend_field2=qq.val();
				var extend_field3=$("input[name='extend_field3']").val();
				var extend_field4=$("input[name='extend_field4']").val();
				var extend_field5=phone.val();
				var sel_question=$("input[name='sel_question']").val();
				var passwd_answer=$("input[name='passwd_answer']").val();
				var agreement = $("input[name='agreement']").val();
				var back_act=$("input[name='back_act']").val();
				var country = 1;
				var province=parseInt($(".select_choose span").eq(0).attr("data-id"));
				var city=parseInt($(".select_choose span").eq(1).attr("data-id"));
				var district=parseInt($(".select_choose span").eq(2).attr("data-id"));
                $.ajax({
                    type: "POST",
                    url: "user.php",
                    async: false,
