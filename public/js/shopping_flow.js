...
{
	if(obj.value==-1){
	   $('#ps_fs').css("display","block");	
	}else{
		$('#ps_wl').val('');
		$('#ps_dh').val('');
		$('#ps_fs').css("display","none");	
	  }
	...
	if(obj.value == 12) {
		alert(obj.getAttribute('data-val')+'。（如果配送范围不在以上区域，请选择其他物流！该配送方式不接受含水剂商品，如商品中含水剂请联系客服人员修改配送方式。）') ;
	}
  /* 2015-4-7 物流配送费用 */
...
/* *
 * 2015-01-05 在线预订 改变支付方式
 */
function selectPayment_online(obj)
{
  if (selectedPayment == obj)
  {
    return;
  ...
  Ajax.call('online_flow.php?step=select_payment', 'payment=' + obj.value, orderSelectedResponse, 'GET', 'JSON');
}
...
function changeSurplus(val)
{
  ...
  Ajax.call('flow.php?step=change_surplus&time='+Math.random(), 'surplus=' + val, changeSurplusResponse, 'GET', 'JSON');
}
 ...
 * 改变余额
 */
function changeSurplus_online(nowsurplus)
{
	selectedSurplus = nowsurplus;

  Ajax.call('online_flow.php?step=change_surplus&time='+Math.random(), 'surplus=' + nowsurplus, changeSurplusResponse, 'GET', 'JSON');
}
...
function changeSurplusResponse(obj)
{
  ...
  else
  {
    try
    {
      ...
      document.getElementById('J_surplus').innerHTML = obj.user_money ;
    }
    catch (ex) { }
...
function checkOrderForm(frm)
{
  ...
  // 2015-01-19
  var gift_staus = document.getElementById("gift_staus").value;
  if(gift_staus == 1){
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  {
	// 2015-01-19
	if(gift_staus == 1){
		if (frm.elements[i].name == 'gift' && frm.elements[i].checked)
		{
			giftSelected = true;
		}else if(frm.elements[i].name == 'gift' && document.getElementById("gift").value==''){
			giftSelected = true;
		}
	}//end
	...
	if(frm.elements[i].value=='-1' && frm.elements[i].checked){
		if(document.getElementById("ps_wl").value=='' && document.getElementById("ps_dh").value==''){
			alert('请填写配送方式');
			return false;
		 }
	}
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  { 
	/* 2014-12-04 检查用户是否选择配送区域 */
	if(frm.elements[i].value == 9 && frm.elements[i].checked){
		if(document.getElementById('area_name').value == 0){
			alert("请选择您的配送区域");
			return false;
		}
	}
	/* 2014-12-12 检查用户是否选择自提库房 */
	if(frm.elements[i].value == 13 && frm.elements[i].checked){
		if(document.getElementById('kf_name').value == 0){
			alert("请选择您的自提库房");
			return false;
		}
	}
...
function checkConsignee(frm)
{
  ...
  {
    err = true;
    //msg.push(tele_not_null); 2014-12-15
  ...
  else
  {
    ...
    {
      err = true;
      msg.push(mobile_invaild);  // 2014-12-15 tele_invaild 改为 mobile_invaild
    }
  }
  ...
  {
    err = true;
    msg.push(tele_invaild);  //2014-12-15  mobile_invaild 改为 tele_invaild
  }
