...
DateInput.DEFAULT_OPTS = {
  ...
  short_day_names: ["周日", "周一", "周二", "周三", "周四", "周五", "周六"],
  start_of_week: 0
};
DateInput.prototype = {
  build: function() {
  ...
  hideIfClickOutside: function(event) {
    if (event.target != this.input[0] && !this.insideSelector(event)) {
    	if(!event.target.className.match(/year_name/) && !event.target.className.match(/month_name/) && !event.target.className.match(/month_nav/) && !event.target.className.match(/year_nav/) && !event.target.className.match(/prev/) && !event.target.className.match(/next/) && !event.target.nodeName.match(/TH/) && !event.target.className.match(/date_selector/) && !event.target.className.match(/unselected_month/)){
      	this.hide();
    	}
    };
  },
