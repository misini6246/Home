...
function tocart(id, product_id) {
    var num = $('#J_dgoods_num_' + id).val();
    if (product_id == 1) {
        num = $('#J_dgoods_num1_' + id).val();
    }
    $.ajax({
        url: '/gwc',
        type: 'post',
        data: {id: id, num: num, product_id: product_id},
        dataType: 'json',
        success: function (data) {
            if (data.error == 0) {
                if (data.type == 0) {
                    ...
                    var num = $('.cart .num').html();
                    console.log(num)
                    console.log(parseInt(num)+1)
                    $('#only_gwc').html(parseInt(num)+1);
                    $('.cart .num').html(parseInt(num)+1);
