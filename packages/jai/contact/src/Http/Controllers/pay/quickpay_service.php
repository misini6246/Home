...
class quickpay_service
{
    ...
    function __construct($args, $service)
    {
        $param_check    = array();
        switch($service)
        {
...
//    function create_html($button)
//    {
//        $html = <<<eot
//	<form style="text-align:center;" id="pay_form" name="pay_form" action="{$this->api_url}" method="post" target="_blank">
//eot;
...
//    }
//2015-12-11 新支付
    function create_html_flow($button)
    {
        $html = <<<eot
	<form style="text-align:center;" id="pay_form" id="pay_form" action="{$this->api_url}" method="post" target="_blank">
eot;
        foreach ($this->args as $key => $value)
        {
            $html .= " <input type=\"hidden\" name=\"{$key}\" id=\"{$key}\" value=\"{$value}\" />\n";
    ...
    {
        $html = <<<eot
	<form style="text-align:center;width:50px;display:inline-block;"  id="pay_form" action="https://unionpaysecure.com/api/Pay.action" method="post" target="_blank">
eot;
        foreach ($this->args as $key => $value)
        {
            $html .= " <input type=\"hidden\" name=\"{$key}\" id=\"{$key}\" value=\"{$value}\" />\n";
