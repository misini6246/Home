...
namespace Jai\Contact\Http\Controllers\union;
...
class secureUtil {
    ...
    function sign(&$params) {
        $log = new PhpLog(config('contact.union.SDK_LOG_FILE_PATH'),'PRC',config('contact.union.SDK_LOG_LEVEL'));
        $log->LogInfo ( '=====签名报文开始======' );
        ...
        // 转换成key=val&串
        $common = new common();
        $params_str = $common->createLinkString ( $params, true, false );
        ...
        // 签名证书路径
        $cert_path = config('contact.union.SDK_SIGN_CERT_PATH');
...
//	echo date('Y-m-d',time());
    ...
    function getCertId($cert_path) {
        ...
        openssl_pkcs12_read ( $pkcs12certdata, $certs, config('contact.union.SDK_SIGN_CERT_PWD') );
        $x509data = $certs ['cert'];
    ...
    function getSignCertId() {
        // 签名证书路径
        return $this->getCertId ( config('contact.union.SDK_SIGN_CERT_PATH') );
    }
    function getEncryptCertId() {
        // 签名证书路径
        return $this->getCertIdByCerPath ( config('contact.union.SDK_ENCRYPT_CERT_PATH') );
    }
    ...
    function getPrivateKey($cert_path) {
        ...
        openssl_pkcs12_read ( $pkcs12, $certs, config('contact.union.SDK_SIGN_CERT_PWD') );
        return $certs ['pkey'];
    }
