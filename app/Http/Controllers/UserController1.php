...
use Jai\Contact\Http\Controllers\PayType;
 ...
 */
    ...
    public function __construct(){
        ...
        if($this->user->is_zq==2){
            ...
            $this->user->zq_rq = $zq_ywy->zq_rq;
            $this->user->zq_ywy = $zq_ywy->zq_ywy;
            $this->user->zq_amount = $zq_ywy->zq_amount;
            $this->user->zq_je = $zq_ywy->zq_je;
            $this->user->zq_has = $zq_ywy->zq_has;
            $this->user->zq_start_date = $zq_ywy->zq_start_date;
            $this->user->zq_end_date = $zq_ywy->zq_end_date;
        }
        $this->arr = [
            'page_title'=>'用户中心-',
            'action'=>'',
    ...
    public function index(Request $request)
    {
        ...
        $yyzz_time = (strtotime($user->yyzz_time)>time() or empty($user->yyzz_time))?1:0;
        $xkz_time = (strtotime($user->xkz_time)>time() or empty($user->xkz_time))?1:0;
        $zs_time = (strtotime($user->zs_time)>time() or empty($user->zs_time))?1:0;
        $yljg_time = (strtotime($user->yljg_time)>time() or empty($user->yljg_time))?1:0;
    ...
    public function orderList(Request $request){
        ...
        $orderList = OrderInfo::where(function($query)use($dates,$keys,$user,$status){
            ...
            ->select('order_id','add_time','order_status','pay_status','shipping_status','order_sn','goods_amount','consignee','jnmj','is_zq')
    ...
    public function orderInfo(Request $request){
        ...
        $order = OrderInfo::with([
            'order_goods'=>function($query){
                $query->select('order_id','goods_id','goods_name','goods_price','goods_number','user_bz');
...
//                </form>';
    ...
    public function useSurplus(Request $request){
        ...
        if($surplus<=0){
            ...
            $orderInfo = OrderInfo::where('is_mhj',0)
                ->select('order_id','order_sn','order_amount','order_status','pay_status','surplus','user_id','is_zq','goods_amount')
                ->findOrfail($orderId);
    ...
    public function zncg(){
        $user =Auth::user()->is_new_user();
        $zncg = DB::table('order_info as oi')
            ->leftJoin('order_goods as og','oi.order_id','=','og.order_id')
            ->leftJoin('goods as g','g.goods_id','=','og.goods_id')
            ->where('oi.user_id',$user->user_id)->where('oi.pay_status',2)->where('oi.parent_id',0)->where('g.goods_id','>',0)
 ...
 */
    public function reg_success(){
        ...
        return view('auth.yhq_msg')->with($assign);
    }
    ...
    public function zq_order_info(Request $request){
        ...
        if($zq_order->pay_status!=2&&$zq_order->order_status==1&&$zq_order->order_amount>0){
            if(abs($order_amount-$zq_order->goods_amount)<0.000001) {
                ...
                $abcpay = $this->button($zq_order, 5, $this->user->is_zq);

                /**
                 * 兴业银行支付
                 */
                $xyyh = $this->button($zq_order, 6, $this->user->is_zq);

                /**
                 * 微信扫码支付
                 */
                $weixin = $this->button($zq_order, 7, $this->user->is_zq);
    ...
    public function youhuiq(){
        ...
        $youhuiq = YouHuiQ::where('user_id',$this->user->user_id)
            ->where('status',0)->where('start','<=',$now)->where('end','>=',$now)
            ->where('enabled',1)
    ...
    public function dzfp(){
        $wldwid1 = $this->user->wldwid1;
        $url = 'http://www.dzfpcx.cn/index.html#/wldwid/8171/'.$wldwid1.'/'.$this->user->mobile_phone;
        return redirect()->to($url);
    }
}