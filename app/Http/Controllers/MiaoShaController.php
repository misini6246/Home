...
//        $this->middleware('user.check', ['only' => ['buy']]);

        //$this->middleware('is_zhongduan', ['only' => ['index', 'buy', 'ms_goods', 'get_cart_goods']]);
        $this->tags = 'miaosha';
        $this->team = collect();
        $this->now = time();
        $this->start = strtotime('2019-10-31 00:00:00');
        $this->end = strtotime('2019-10-31  00:00:00');
        $this->area_xg = [
            8 => '贵州',
            26 => '四川',
            28 => '西藏',
        ...
        $this->assign['page_title'] = '11月1日秒杀-';
        $this->assign['now'] = $this->now;
        $cs_arr = cs_arr();
        //$cs_arr = [];
        if (auth()->check() && in_array(auth()->user()->user_id, $cs_arr)) {
            $this->start = strtotime('2019-10-31 23:50:00');
            $this->team[] = $this->get_team([strtotime('2019-11-01 00:00:00'), strtotime('2019-11-01 00:30:00'), 1]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 08:00:00'), strtotime('2019-11-01 00:30:00'), 2]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 11:00:00'), strtotime('2019-11-01 00:30:00'), 3]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 15:00:00'), strtotime('2019-11-01 00:30:00'), 4]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 18:00:00'), strtotime('2019-11-01 00:30:00'), 5]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 22:00:00'), strtotime('2019-11-01 00:30:00'), 6]);
        } else {
            $this->team[] = $this->get_team([strtotime('2019-11-01 00:00:00'), strtotime('2019-11-01 00:30:00'), 1]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 08:00:00'), strtotime('2019-11-01 00:30:00'), 2]);
            $this->team[] = $this->get_team([strtotime('2019-11-01 11:00:00'), strtotime('2019-11-01 00:30:00'), 3]);
    ...
    public function miaosha_num(Request $request)
    {
        $num = intval($request->input('mun'));
        $id = intval($request->input('id'));
        if($request->input('ku') == 1){
            Cache::store('miaosha')->tags(['miaosha', 'goods', $this->tags, 'kc'])->forever($id, $num);//设置库存

        }
            Cache::store('miaosha')->tags(['miaosha', 'goods', $this->tags, 'kc'])->decrement($id,$num);//减少库存
...
//网站
    public function index(Request $request)
    {
      ...
      Cache::store('miaosha')->flush();
        ...
        foreach ($this->team as $k => $v) {
            ...
            foreach($new as $v){
                if($v->start < time() && $v->end > time()){
                    ...
                    $v->goods->map(function ($itme){
                        $a = explode('.',$itme['real_price']);
                        $s = mb_strlen($a[0]) == 1 ? '?.':'??.';
                        $w = @mb_strlen($a[1]) ==1? '?':'??';
                        return $itme->real_price = $s.$w;
                    });
                    $v->kaishi = 0;
                }
            }
        ...
        if(request()->route()->getName()=='miaosha111'){
            $this->assign['page_title'] = '11.1秒杀';
            // dd($this->assign);
            return view('hd.111.miaosha', $this->assign);
        }
    ...
    public function buy(Request $request)
    {
        ...
        if ($this->user->ls_review== 0 || $this->user->ls_review_7day == 1 || ($this->user->ls_review_7day == 1 && $this->user->day7_time < time())) {
        ...
        $id = intval($request->input('id'));
        if ($this->user->city == 322 && $id != 8229) {
            ajax_return('该活动针对成都以外地区终端客户，成都地区活动时间9月13号敬请期待！', 1);
        }
//        if ($this->user->city != 322) {
//            ajax_return('该活动针对成都地区终端客户', 1);
//       }

            $this->user->yyzz_time=strtotime($this->user->yyzz_time);
            $this->user->xkz_time=strtotime($this->user->xkz_time);
            $this->user->zs_time=strtotime($this->user->zs_time);
            $this->user->yljg_time=strtotime($this->user->yljg_time);

            // 2014-11-26 采购、提货、收货委托书及身份证复印件

            $time=time();

          if( $this->user->yyzz_time =='' || $this->user->yyzz_time < $time) {
                ajax_return('你的营业执照已过期，请尽快重新邮寄', 1);
            }

            if( $this->user->xkz_time =='' || $this->user->xkz_time < $time) {
                ajax_return('你的药品经营许可证已过期，请尽快重新邮寄', 1);
            }

            if( $this->user->zs_time =='' || $this->user->zs_time < $time) {
                ajax_return('你的GSP证书已过期，请尽快重新邮寄', 1);
            }
            if( $this->user->yljg_time =='' || $this->user->yljg_time < $time) {
                ajax_return('你的医疗机构执业许可证已过期，请尽快重新邮寄', 1);
            }
  ...
  /*      $this->user->yyzz_time=strtotime($this->user->yyzz_time);
        ...
        if ($id == 30692 && $this->user->user_rank != 5) {
            ajax_return('该商品仅限诊所购买', 1);
        }
        ...
        if (!empty($goods_info->erp_shangplx) && !in_array($goods_info->erp_shangplx, $this->user->jyfw)) {
            ajax_return("您提供的资质不具备购买" . $goods_info->erp_shangplx . "的权限，如需购买请联系客服人员", 1);
        }

        if (empty($goods_info->erp_shangplx) && $this->user->is_only_buy == 1) {
            ajax_return("您提供的资质不具备购买" . $goods_info->goods_name . "的权限，如需购买请联系客服人员", 1);
        }
        if ($goods_info->is_mhj == 1 && $this->user->mhj_number == 0) {
            ajax_return('不能购买麻黄碱商品', 1);
        }
        if ($goods_info->is_mhj == 1 && $this->user->is_mhj_hz == 0) {
            ajax_return('有麻黄碱订单未收到回执，应GSP要求请将回执盖章拍照发给您的专属客服，补交回执后即可购买，如有疑问请联系客服或拨打电话4006028262！', 1);
        }
...
//        }
        ...
        if (!empty($cart_goods)) {
            ...
            foreach ($cart_goods as $k => $v) {
                ...
                if (isset($quchong[$v->goods_id])) {
                    ...
                    if ($v->goods_id == 30692 && $this->user->user_rank != 5) {
                        unset($cart_goods[$k]);
    ...
    public function ms_goods(Request $request)
    {
        ...
        if (in_array($this->user->user_id, [13960, 18810])) {
            return redirect()->route('index');
        }
        ...
        foreach ($this->goods_arr as $k => $v) {
            $v['id'] = $k;
            $v['tags'] = $this->tags;
            $v['gwc'] = isset($result['kc'][$k]) ? $result['kc'][$k] : 0;
...
//        }
        ...
        $gn = [3497, 1218, 9840, 11088, 8781, 18059, 714, 814, 6127, 1210, 14417, 3076, 18014, 7262, 9838,
            22825, 8780, 18036, 4088, 2150, 912, 5637, 8229, 18010, 17921, 18018, 4098, 19486, 23916, 5326, 21887,18856,16513];
        $arr = [];
        foreach ($gn as $k => $v) {
