...
class Cart
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if($goods_info->ls_gg > 0) {

            if($goods->number < $goods_info->ls_gg) {
                $goods->number = $goods_info->ls_gg ;
            }

            /*
            if($ls_num['goods_number']+$num > $goods['ls_ggg']){
                $GLOBALS['err']->add('购物车本商品的数量已经超过单张订单最高限购限购数量', ERR_NO_BASIC_GOODS);
